...
dataloader_train = torch.utils.data.DataLoader(camvid_dataset_train, batch_size=batch_size, shuffle=True, num_workers=0)
...
dataloader_val = torch.utils.data.DataLoader(camvid_dataset_val, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
...
dataloader_test = torch.utils.data.DataLoader(camvid_dataset_test, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
...
def eval_model(model, dataloader, device, save_pred=False):
    ...
    class_intersections, class_unions, class_pixel_counts = np.zeros(num_classes), np.zeros(num_classes), np.zeros(num_classes)
    all_preds = []

    if save_pred:
        print("save pred is true")
        pred_list = []
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            total_correct_pixels += (pred_np == labels_np).sum()
            total_pixels += pred_np.size

            for cls in range(num_classes):
                pred_cls = pred_np == cls
                labels_cls = labels_np == cls

                intersection = (pred_cls & labels_cls).sum()
                union = pred_cls.sum() + labels_cls.sum() - intersection
                class_pixel_counts[cls] += labels_cls.sum()
                
                class_intersections[cls] += intersection
                class_unions[cls] += union

        pixel_acc = total_correct_pixels / total_pixels
        mean_iou = np.mean(class_intersections / (class_unions + 1e-10))  # Avoid division by zero
        freq_iou = np.sum((class_pixel_counts / total_pixels) * (class_intersections / (class_unions + 1e-10)))
        loss = loss_fn(outputs, labels)
